...
%% Set up movie
if SAVE_MOVIE
    vidObj = VideoWriter('outputs/EncounterMovieWalls.avi');
    vidObj.FrameRate=30;
    open(vidObj);
end
...
hold on;
if PLOT_MAP
    [x,y] = map.getWallCoords();
    h_map = plot(x, y, 's','MarkerSize',12,'Color',[0.7 0.7 0.7], 'MarkerFaceColor',[0.7 0.7 0.7]);
end
if PLOT_AGENTS
...
for t= 0:sim_step:sim_end
    ...
    % -- edit: -- there must be a straightline between this waypoint and the
    % agent without crossing a wall
    for idx=1:N_moving
        agent_id = agents_moving(idx);
        % loop until we find a viable waypoint
        waypoint_found = false;
        while ~waypoint_found
            candidate = [GRID_X*rand GRID_Y*rand];
            if map.isPathFree( agents_coords(agent_id,:), candidate )
                % we found a viable path
                agents_waypoints(agent_id,:) = candidate;
                waypoint_found = true;
            end
...
if SAVE_MOVIE
    vidObj = VideoWriter('outputs/estimateCorrectionWalls.avi');
    vidObj.FrameRate=30;
    open(vidObj);
end
...
%saveplot('figures/error_correctionWalls');
